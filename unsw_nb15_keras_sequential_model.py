# -*- coding: utf-8 -*-
"""
Created on Mon Aug 10 21:49:33 2020
...
import unsw_nb15_data_set_wrangling as un15w
...
def load_data_set(data_set_tag="0K"):
    """Loads the data from the UNSW_NB15_<tag>.csv file
    Parameters
    ----------
    data-set-tag : str
    ...
    """
    return un15w.get_X_raw_y_raw(data_set_tag)

def make_up_y(y_raw=None, kind_of_columns=''):
    ...
    if 'attack_cat' == kind_of_columns:
# =============================================================================
#         y_attack_cat = y_raw[['attack_cat']].copy()
#         y_attack_cat.fillna('bening', inplace=True)
#         
#         y_label_encoder = LabelEncoder()
#         y_label_encoder.fit(y_attack_cat['attack_cat'])
#         y_encoded = y_label_encoder.transform(y_attack_cat)
#     
#         # Converts a class vector (integers) to binary class matrix.
#         y_binary_class_matrix = np_utils.to_categorical(y_encoded['attack_cat'])
# =============================================================================
        return un15w.get_wrangled_attack_cat(y_raw)
    
    elif 'Label' == kind_of_columns:
        # 0 = benign 
        # 1 = any attack
        y_label_encoder = LabelEncoder()
        y_label_encoder.fit(y_raw[['Label']])
...
def make_up_X(X_raw=None, kind_of_columns=''):
    ...
    """
    # getting scalars features
    scalars_df = un15w.get_wrangled_column(X_raw, column_name='scalars')
    
    dsport = un15w.get_wrangled_column(X_raw, column_name='dsport')
    proto_df = un15w.get_wrangled_column(X_raw, column_name='proto')
    state_df = un15w.get_wrangled_column(X_raw, column_name='state')
    
    selected_data_set_npa = []

    # selecting features to be use as input (scalars, categorical, or both)
    if 'categorical' == kind_of_columns:
        selected_data_set_df = pd.concat([dsport, 
                                          proto_df,
                                          state_df], axis=1, sort=False)
...
def main():
    ...
    y = make_up_y(y_raw, kind_of_columns='attack_cat')    
